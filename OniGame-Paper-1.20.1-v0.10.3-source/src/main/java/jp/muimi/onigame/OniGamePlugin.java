...
public final class OniGamePlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private Location activeLobby, activeExit, activePlayerSpawn; private long botNextAbilityAt, botRestUntil, oniChaseStartedAt, nextBloodScentAt; private double botStamina=20; private int botDakkoSkillCycle;
    ...
    @Override public boolean onCommand(CommandSender sender,Command command,String label,String[] a){
        ...
        if(a[0].equalsIgnoreCase("skill")&&sender instanceof Player p){PlayerSkill sk=a.length>1?PlayerSkill.parse(a[1]):null;if(sk==null){msg(p,"sprint / invisible / smoke / strike / heal から選択してください。");return true;}selectedSkill.put(p.getUniqueId(),sk);msg(p,"スキルを &b"+sk.display+" &fに設定しました。");return true;}
    ...
    private void start(OniType forced,CommandSender starter,boolean playerTest){
        ...
        List<Location> lootChests=selectLootChestLocations(hearts,ps);
    ...
    private Material skillMaterial(PlayerSkill s){return switch(s){case SPRINT->Material.FEATHER;case INVISIBLE->Material.GLASS_BOTTLE;case SMOKE->Material.INK_SAC;case ONI_STRIKE->Material.BLAZE_ROD;case HEAL->Material.GOLDEN_APPLE;case OBSESSION->Material.SPECTRAL_ARROW;};}
    ...
    private void openSkillMenu(Player p){
        ...
        menu.setItem(9,menuItem(Material.FEATHER,"&b&l疾走","menu_skill:SPRINT","&7効果: 5秒間高速移動","&7クールダウン: 25秒",current==PlayerSkill.SPRINT?"&a✔ 選択中":"&eクリックして選択"));
        ...
        menu.setItem(18,menuItem(Material.GLISTERING_MELON_SLICE,"&a&lプリセット: 救護役","menu_preset:MEDIC","&8メイン: &f治療","&8パッシブ: &f集中 / 絆","&8&m--------------------","&7治療完了時に周囲の仲間も回復","","&eクリックで一括設定"));
        ...
        int[] passiveSlots={36,38,40,42,44,46,52,53};PassiveSkill[] values=PassiveSkill.values();for(int i=0;i<values.length;i++){PassiveSkill passive=values[i];menu.setItem(passiveSlots[i],menuItem(passiveMaterial(passive),"&a&l"+passive.display,"menu_passive:"+passive.name(),"&7"+passive.description,passives.contains(passive)?"&a✔ 選択中":"&eクリックして選択"));}
    ...
    private void updateOniBot(){
        ...
        Player target=players.stream().filter(id->!dead.contains(id)&&!escaped.contains(id)).map(Bukkit::getPlayer).filter(Objects::nonNull).filter(p->p.getWorld().equals(bot.getWorld())).min(Comparator.comparingDouble(p->p.getLocation().distanceSquared(bot.getLocation()))).orElse(null);
    ...
    private void updatePlayerBots(){
        ...
        for(UUID id:new HashSet<>(playerBots)){
            ...
            if(!fleeing&&brokenHearts<totalHearts&&distance<=2.6){bot.setVelocity(new Vector(0,bot.getVelocity().getY(),0));updatePlayerBotStamina(id,false);advanceHeartBot(id,bot,destination.getBlock());continue;}if(!fleeing)clearPlayerBotWork(id);
            if(hunting&&hunter!=null&&distance<=getConfig().getDouble("player-bot.hunt-attack-range",2.4)){bot.setVelocity(new Vector(0,bot.getVelocity().getY(),0));updatePlayerBotStamina(id,false);attackOniWithPlayerBot(id,bot,hunter);continue;}
    ...
    private void movePlayerBot(LivingEntity bot,Location destination,double step){
        Vector wanted=destination.toVector().subtract(bot.getLocation().toVector()).setY(0);if(wanted.lengthSquared()<.01){bot.setVelocity(bot.getVelocity().multiply(.45));return;}wanted.normalize();
        Vector separation=new Vector();double radius=getConfig().getDouble("player-bot.movement.separation-radius",2.2);for(UUID otherId:playerBots){if(otherId.equals(bot.getUniqueId())||!(Bukkit.getEntity(otherId) instanceof LivingEntity other)||!other.getWorld().equals(bot.getWorld()))continue;Vector away=bot.getLocation().toVector().subtract(other.getLocation().toVector()).setY(0);double length=away.length();if(length>0&&length<radius)separation.add(away.normalize().multiply((radius-length)/radius));}if(separation.lengthSquared()>0)wanted.add(separation.multiply(getConfig().getDouble("player-bot.movement.separation-strength",.8))).normalize();
        Vector clear=findPlayerBotClearDirection(bot,wanted);double speed=Math.min(getConfig().getDouble("player-bot.movement.max-velocity",.28),Math.max(.08,step*getConfig().getDouble("player-bot.movement.step-to-velocity",.36)));Vector old=bot.getVelocity().setY(0);double smoothing=Math.max(0,Math.min(1,getConfig().getDouble("player-bot.movement.steering-smoothing",.42)));Vector horizontal=old.multiply(1-smoothing).add(clear.multiply(speed*smoothing));if(horizontal.length()>speed)horizontal.normalize().multiply(speed);
        Location last=playerBotLastLocation.put(bot.getUniqueId(),bot.getLocation().clone());boolean stuck=last!=null&&last.getWorld().equals(bot.getWorld())&&last.distanceSquared(bot.getLocation())<.015;int stuckTicks=stuck?playerBotStuckTicks.getOrDefault(bot.getUniqueId(),0)+1:0;playerBotStuckTicks.put(bot.getUniqueId(),stuckTicks);double y=bot.getVelocity().getY();if(bot.isOnGround()&&(isPlayerBotBlocked(bot,clear,1.0)||stuckTicks>=getConfig().getInt("player-bot.movement.stuck-updates-before-jump",3))){y=getConfig().getDouble("player-bot.movement.jump-velocity",.42);playerBotStuckTicks.put(bot.getUniqueId(),0);}bot.setVelocity(new Vector(horizontal.getX(),y,horizontal.getZ()));float yaw=(float)Math.toDegrees(Math.atan2(-horizontal.getX(),horizontal.getZ()));bot.setRotation(yaw,bot.getLocation().getPitch());
    }
